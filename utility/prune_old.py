...
def prune_by_noise(model,model_unprune, mask, percent,test_loader,criterion, noise_type ,prior_sigma=1.0, 
  ...
  #          ipdb.set_trace()
            ...
            for param in model_copy.parameters():
                param.requires_grad = False
            for param in model_unprune.parameters():
        ...
        #alive = torch.nonzero(param.data.abs(), as_tuple=True) # flattened array of nonzero values
                ...
                if kl:
                  ...
                  #  kl_loss = torch.norm(torch.sigmoid(p),0.3)
                   # kl_loss = torch.norm(torch.sigmoid(p),1)/torch.norm(torch.sigmoid(p),2)
                   # kl_loss = (torch.sigmoid(p)*(1.5-1*torch.sigmoid(p))).sum()
                if reduce_op:
                    kl_loss /= num_params
       ...
       # scheduler.step(total_loss_accum)
        ...
        print(f"Average batch original loss after noise: {batch_original_loss_after_noise_accum / len(test_loader):.6f}")
        if kl:
            print(f"Average KL loss: {kl*kl_loss_accum / len(test_loader):.6f}")
